...
class TemplateController
{
	...
	public function header($openTag, $closeTag) {
		$sHeader = $this->site->getHeader();
		if (!empty($sHeader)){
			return $openTag . $sHeader . $closeTag;
		}
		return "";
	}
	...
	public function globalScript($sPath) {
		return '<script>' . file_get_contents(JAB_ROOT . '/public/' . $sPath) . '</script>';
	}
	
	public function globalStyle($sPath) {
		return '<style>' . file_get_contents(JAB_ROOT . '/public/' . $sPath) . '</style>';
	}
	...
	public function vendor($sPath) {
		return file_get_contents(JAB_ROOT . '/vendor/' . $sPath);
	}
